...
namespace Commercial_Office.Controllers
{
    ...
    public class OfficeController : Controller
    {
        ...
        public async Task<ActionResult<IList<OfficeDTO>>> GetOffices()
        {
            try
            {
                IList<OfficeDTO> offices = await this._officeService.GetAll();

                if (offices.IsNullOrEmpty()) {
                    return Ok("No hay oficinas en el sistema");
                }
        ...
        public async Task<ActionResult<OfficeDTO>> GetOffice(string id) {
            try
            {
                var officeDTO = await this._officeService.GetOffice(id);
                return Ok(officeDTO);
            }
        ...
        public async Task<ActionResult<OfficeDTO>> CreateOffice(OfficeDTO officeDTO)
        {
            try
            {
                await _officeService.CreateOffice(officeDTO);
        ...
        public async Task<ActionResult<IList<AttentionPlaceDTO>>> UpdateOffice(string officeId, [FromBody] IList<AttentionPlaceDTO> places)
        {

            try
            {
                await _officeService.UpdateOffice(officeId, places);
        ...
        /// <param name="officeId"> Identificador de la oficina a registrar el usuario</param>
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<string>> RegisterUser(string userId, string officeId)
        {
            try
            {
                await _officeService.RegisterUser(userId, officeId);
        ...
        /// </summary>
        /// <param name="officeId"> Identificador de la oficina donde se encuentra el puesto</param>
